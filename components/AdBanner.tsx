...
export function AdBanner() {
  ...
  // ✅ P2: `fixed bottom-0` → takes NO flow space. No CLS.
  // z-30 sits below mobile drawer backdrop (z-[45]) to avoid collision.
  return (
    <div
      role="complementary"
      aria-label="Sponsored message"
      className="fixed bottom-0 left-0 right-0 z-30 md:hidden pb-[env(safe-area-inset-bottom)]"
    >
      <div className="bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-gray-200 dark:border-gray-700 p-3 shadow-lg">
        <div className="flex items-center justify-between gap-3 max-w-screen-xl mx-auto">
          ...
          <button
            onClick={handleClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500"
